...
export class SSSA {
          ...
          if (i !== j) {
            product =
              ((product +
                ...
                1) %
                this.fieldSize) -
              1; // to make sure it's not negative
          }
        }
  ...
  async getPointsForChunks(secret, numShares, threshold) {
    let allPoints = [];
  ...
  sharesToBin(sharesMatrix) {
    ...
    return { x: sharesMatrix.x, y: output };
  }
